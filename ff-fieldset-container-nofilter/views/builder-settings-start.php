...
<p class="frm-mt-xs">
    <label for="legend_heading_<?php echo esc_attr( $field_id ); ?>">
        <?php esc_html_e( 'Legend heading level', 'ff-fieldset-container-nofilter' ); ?>
	<span class="frm_help frm_icon_font frm_tooltip_icon"
          title="<?php esc_attr_e( 'Optionally wrap the legend text in a heading level (H2–H6). Choose None to output plain legend text.', 'ff-fieldset-container-nofilter' ); ?>"
          aria-label="<?php esc_attr_e( 'Optionally wrap the legend text in a heading level (H2–H6). Choose None to output plain legend text.', 'ff-fieldset-container-nofilter' ); ?>">
    </span>
    </label>
    <select name="field_options[legend_heading_<?php echo esc_attr( $field_id ); ?>]" id="legend_heading_<?php echo esc_attr( $field_id ); ?>">
        <?php
        $options = array( '' => __( 'None', 'ff-fieldset-container-nofilter' ), 'h2' => 'H2', 'h3' => 'H3', 'h4' => 'H4', 'h5' => 'H5', 'h6' => 'H6' );
...
<p class="frm-mt-xs">
    <label for="legend_visible_<?php echo esc_attr( $field_id ); ?>">
        <input type="checkbox"
               ...
               <?php checked( $legend_visible, '1' ); ?> />
        <?php esc_html_e( 'Show legend visually', 'ff-fieldset-container-nofilter' ); ?>
	<span class="frm_help frm_icon_font frm_tooltip_icon"
          title="<?php esc_attr_e( 'When unchecked, the legend will be hidden visually but remain accessible to screen readers.', 'ff-fieldset-container-nofilter' ); ?>"
          aria-label="<?php esc_attr_e( 'When unchecked, the legend will be hidden visually but remain accessible to screen readers.', 'ff-fieldset-container-nofilter' ); ?>">
    </span>
    </label>
