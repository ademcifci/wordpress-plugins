...
class FF2_Fieldset_Start extends FrmFieldType {
    ...
    protected function include_form_builder_file() {
        return FF2_FIELDSET_DIR . 'views/builder-field-start.php';
    }

    public function show_primary_options( $args ) {
        $field = $args['field'];
        include FF2_FIELDSET_DIR . 'views/builder-settings-start.php';
    ...
    public function front_field_input( $args, $shortcode_atts ) {
        ...
        // Allow last-minute filtering for advanced control.
        if ( function_exists( 'apply_filters' ) ) {
            $classes = apply_filters( 'ff2_fieldset_start_container_classes', $classes, $field );
        }

        $html  = '<fieldset' . $id_attr . ' class="' . esc_attr( $classes ) . '">';
        $html .= '<legend' . $legend_class . '>';
        if ( $legend_heading ) {
            $html .= '<' . $legend_heading . '>' . esc_html( $legend_text ) . '</' . $legend_heading . '>';
        } else {
            $html .= esc_html( $legend_text );
        }
        $html .= '</legend>';
        if ( ! empty( $description ) ) {
            $html .= '<div class="frm_description">' . wp_kses_post( $description ) . '</div>';
        }
        return $html;
    }
}
